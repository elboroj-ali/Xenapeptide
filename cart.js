...
window.Cart = (function(){
  ...
  const WHATSAPP_NUMBER = '491521676741';
  ...
  function add(product, qty){
    ...
    if(existing){ existing.qty += qty; }
      ...
      items.push({
        ...
        price: product.price, image: product.image, qty: qty
      });
    }
    write(items);
  }
  ...
  function total(){
    return read().reduce((sum,i)=> sum + (i.price!=null ? i.price*i.qty : 0), 0);
  }
  ...
  function orderText(){
    ...
    let lines = ['Hello XENA PEPTIDE, I would like to order:', ''];
    items.forEach(i=>{
      const priceStr = i.price!=null ? ` — €${(i.price*i.qty).toFixed(0)}` : '';
      lines.push(`• ${i.name} ${i.strength} x${i.qty}${priceStr}`);
    });
    const hasPrices = items.some(i=>i.price!=null);
    if(hasPrices) lines.push('', `Total: €${total().toFixed(0)}`);
  ...
  return { read, add, setQty, remove, clear, count, total, updateBadge, orderText, whatsappLink, telegramLink };
})();
